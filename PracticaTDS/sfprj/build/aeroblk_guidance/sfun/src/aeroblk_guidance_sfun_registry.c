...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
 const mxArray * prhs[] )
{
  extern unsigned int sf_aeroblk_guidance_process_check_sum_call( int nlhs,
   mxArray * plhs[], int nrhs, const mxArray * prhs[] );

  if(sf_aeroblk_guidance_process_check_sum_call(nlhs,plhs,nrhs,prhs)) return 1;
  return 0;
}
unsigned int sf_process_autoinheritence_call( int nlhs, mxArray * plhs[], int
 nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_aeroblk_guidance_autoinheritance_info( int nlhs,
   mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if(sf_aeroblk_guidance_autoinheritance_info(nlhs,plhs,nrhs,prhs)) return 1;
  return 0;
}
...
extern unsigned int sf_aeroblk_guidance_method_dispatcher(SimStruct *S, const
 char *chartName, int_T method, void *data);
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
 char *chartName, int_T method, void *data)
{
  if(sf_aeroblk_guidance_method_dispatcher(simstructPtr,chartName,method,data))
  return 1;
  return 0;
}
extern void aeroblk_guidance_terminator(void);
void sf_machine_global_terminator(void)
{
  if(sfMachineGlobalTerminatorCallable) {
    ...
    aeroblk_guidance_terminator();
    sf_debug_terminate();
  }
  return;
}
extern void aeroblk_guidance_initializer(void);
extern void aeroblk_guidance_debug_initialize(void);
void sf_machine_global_initializer(void)
{
  if(sfMachineGlobalInitializerCallable) {
    ...
    aeroblk_guidance_debug_initialize();
    aeroblk_guidance_initializer();
  }
  return;
}
