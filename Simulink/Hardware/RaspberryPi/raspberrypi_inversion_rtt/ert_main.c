...
void baseRateTask(void *arg)
{
  ...
  while ((rtmGetErrorStatus(raspberrypi_inversion_M) == (NULL)) &&
         !rtmGetStopRequested(raspberrypi_inversion_M) ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(raspberrypi_inversion_M->extModeInfo, 1,
        &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(raspberrypi_inversion_M, TRUE);
      }

      if (rtmGetStopRequested(raspberrypi_inversion_M) == TRUE) {
    ...
    {
      boolean_T rtmStopReq = FALSE;
      rtExtModeOneStep(raspberrypi_inversion_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(raspberrypi_inversion_M, TRUE);
      }
    }

    raspberrypi_inversion_output();
    ...
    {                                  /* Sample time: [0.1s, 0.0s] */
      rtExtModeUpload(0, raspberrypi_inversion_M->Timing.taskTime0);
    }

    raspberrypi_inversion_update();
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(raspberrypi_inversion_M, 0);
  ...
  raspberrypi_inversion_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(raspberrypi_inversion_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = FALSE;
    rtExtModeWaitForStartPkt(raspberrypi_inversion_M->extModeInfo, 1,
      &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(raspberrypi_inversion_M, TRUE);
    }
  ...
  if (rtmGetErrorStatus(raspberrypi_inversion_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(raspberrypi_inversion_M));
  }
  ...
  /* Terminate model */
  raspberrypi_inversion_terminate();
  return 0;
}
