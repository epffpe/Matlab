...
void baseRateTask(void *arg)
{
  ...
  while ((rtmGetErrorStatus(beagleboard_inversion_M) == (NULL)) &&
         !rtmGetStopRequested(beagleboard_inversion_M) ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(beagleboard_inversion_M->extModeInfo, 1,
        &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(beagleboard_inversion_M, TRUE);
      }

      if (rtmGetStopRequested(beagleboard_inversion_M) == TRUE) {
    ...
    {
      boolean_T rtmStopReq = FALSE;
      rtExtModeOneStep(beagleboard_inversion_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(beagleboard_inversion_M, TRUE);
      }
    }

    beagleboard_inversion_output();
    ...
    {                                  /* Sample time: [0.1s, 0.0s] */
      rtExtModeUpload(0, beagleboard_inversion_M->Timing.taskTime0);
    }

    beagleboard_inversion_update();
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(beagleboard_inversion_M, 0);
  ...
  beagleboard_inversion_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(beagleboard_inversion_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = FALSE;
    rtExtModeWaitForStartPkt(beagleboard_inversion_M->extModeInfo, 1,
      &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(beagleboard_inversion_M, TRUE);
    }
  ...
  if (rtmGetErrorStatus(beagleboard_inversion_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(beagleboard_inversion_M));
  }
  ...
  /* Terminate model */
  beagleboard_inversion_terminate();
  return 0;
}
