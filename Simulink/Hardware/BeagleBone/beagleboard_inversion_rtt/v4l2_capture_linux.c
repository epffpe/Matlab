        ...
        case IO_METHOD_MMAP:
            ...
            if (xioctl(h->fd, VIDIOC_DQBUF, &buf) == -1) {
                switch (errno) {
                    ...
                    case EIO:
                    default:
                        MW_ERROR_EXIT2("Error: Failure reading image buffer. System returned (%d): %s.", 
             ...
             if (xioctl(h->fd, VIDIOC_QBUF, &buf) == -1) {
                 MW_ERROR_EXIT3("Error: Failure returning video buffer to device %s. System returned (%d): %s.", 
                         h->devName, errno, strerror(errno));
             }
             break;
    }
