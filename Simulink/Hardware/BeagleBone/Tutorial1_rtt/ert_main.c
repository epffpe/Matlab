...
void baseRateTask(void *arg)
{
  ...
  while ((rtmGetErrorStatus(Tutorial1_M) == (NULL)) && !rtmGetStopRequested
         (Tutorial1_M) ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(Tutorial1_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(Tutorial1_M, TRUE);
      }

      if (rtmGetStopRequested(Tutorial1_M) == TRUE) {
    ...
    {
      boolean_T rtmStopReq = FALSE;
      rtExtModeOneStep(Tutorial1_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(Tutorial1_M, TRUE);
      }
    }
    ...
    {                                  /* Sample time: [0.01s, 0.0s] */
      rtExtModeUpload(0, Tutorial1_M->Timing.taskTime0);
    }
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(Tutorial1_M, 0);
  ...
  Tutorial1_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(Tutorial1_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = FALSE;
    rtExtModeWaitForStartPkt(Tutorial1_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(Tutorial1_M, TRUE);
    }
  }
  ...
  if (rtmGetErrorStatus(Tutorial1_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(Tutorial1_M));
  }
